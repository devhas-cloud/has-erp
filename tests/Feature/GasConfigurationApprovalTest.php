...
class GasConfigurationApprovalTest extends TestCase
{
    use RefreshDatabase;

    private Division $gas;

    private Division $other;

    private User $creator;

    private User $gasColleague;
    ...
    protected function setUp(): void
    {
        ...
        $this->creator = User::create([
            ...
            'division_id' => $this->gas->id,
            'role' => 'User',
        ]);

        $this->gasColleague = User::create([
            'username' => 'riki',
            'email' => 'riki@has.com',
            'password' => bcrypt('secret'),
            'division_id' => $this->gas->id,
        ...
        $this->opportunity = Opportunity::create([
            'opportunity_name' => 'Gas Treatment Tuban',
            'account_companies_id' => $this->company->id,
        ...
        foreach ([$this->creator, $this->gasColleague, $this->otherUser] as $user) {
    ...
    private function createQuotation(User $asUser): QuoteConfiguration
    {
        ...
        $response = $this->actingAs($asUser)->postJson(route('gas-configuration.store'), [
            'task_id' => $task->id,
            'date' => '2026-08-20',
    ...
    {
        $task = $this->createQuoteTask();
        $this->actingAs($this->creator)->postJson(route('gas-configuration.store'), [
            'task_id' => $task->id,
            'parameter_note' => 'pH, Ammonia, COD, TSS dan Debit',
        ...
        $this->assertSame('Gas', $quotation->division?->division_name);
    }
    ...
    public function test_fetch_task_returns_derived_data(): void
    {
        ...
        $response = $this->actingAs($this->creator)
            ->getJson(route('gas-configuration.fetch-task').'?task_id='.$task->id)
            ->assertOk();
    ...
    public function test_creator_from_gas_division_cannot_approve_own_quotation(): void
    {
        $quotation = $this->createQuotation($this->creator);
        $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.submit', $quotation->id))
            ->assertOk();

        // Aturan: pembuat tidak bisa approve dokumennya sendiri
        $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.approve', $quotation->id))
            ->assertForbidden();
    ...
    public function test_other_user_from_same_division_can_approve(): void
    {
        ...
        $this->actingAs($this->gasColleague)
            ->postJson(route('gas-configuration.approve', $quotation->id))
            ->assertOk();

        $fresh = $quotation->fresh();
        $this->assertSame('approved', $fresh->status);
        $this->assertSame($this->gasColleague->id, $fresh->final_checked_by);
    ...
    public function test_reject_requires_reason_and_same_division_approver(): void
    {
        ...
        $this->actingAs($this->gasColleague)
            ->postJson(route('gas-configuration.reject', $quotation->id), [])
            ->assertUnprocessable();

        // Reject oleh user divisi lain => forbidden
        $this->actingAs($this->otherUser)
            ->postJson(route('gas-configuration.reject', $quotation->id), ['approval_note' => 'Harga terlalu tinggi'])
            ->assertForbidden();
    ...
    public function test_rejected_quotation_can_be_revised_and_resubmitted(): void
    {
        ...
        $this->actingAs($this->creator)
            ->get(route('gas-configuration.edit', $quotation->id))
            ->assertRedirect(route('gas-configuration.index'));

        // Buat revisi => versi baru (Draft)
        $revisionResponse = $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.revise', $quotation->id))
            ->assertOk();
        ...
        $this->actingAs($this->creator)
            ->get(route('gas-configuration.edit', $revision->id))
            ->assertOk();

        $task = $this->createQuoteTask();
        $this->actingAs($this->creator)
            ->putJson(route('gas-configuration.update', $revision->id), [
                'task_id' => $task->id,
        ...
        $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.submit', $revision->id))
            ->assertOk();
    ...
    public function test_approved_quotation_cannot_be_edited_or_deleted(): void
    {
        ...
        $this->actingAs($this->creator)
            ->putJson(route('gas-configuration.update', $quotation->id), [
        ...
        $this->actingAs($this->creator)
            ->deleteJson(route('gas-configuration.destroy', $quotation->id))
            ->assertStatus(422);
    }
    ...
    public function test_approved_is_locked_unlocked_and_revised(): void
    {
        ...
        $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.unlock', $quotation->id))
            ->assertForbidden();
    ...
    {
        $quotation = $this->createQuotation($this->creator);
        $this->actingAs($this->creator)->postJson(route('gas-configuration.submit', $quotation->id))->assertOk();
        $this->actingAs($this->gasColleague)->postJson(route('gas-configuration.approve', $quotation->id))->assertOk();
        $this->actingAs($this->gasColleague)->postJson(route('gas-configuration.unlock', $quotation->id))->assertOk();

        $rev = $this->actingAs($this->creator)->postJson(route('gas-configuration.revise', $quotation->id))->json('id');
        $revision = QuoteConfiguration::findOrFail($rev);

        $this->actingAs($this->creator)->postJson(route('gas-configuration.submit', $revision->id))->assertOk();
        $this->actingAs($this->gasColleague)->postJson(route('gas-configuration.approve', $revision->id))->assertOk();
        ...
        $versions = $this->actingAs($this->creator)
            ->getJson(route('gas-configuration.versions', $revision->id))
            ->assertOk()
            ->json('versions');
    ...
    {
        UserAccessControl::where('user_id', $this->gasColleague->id)
            ->first()
        ...
        $this->actingAs($this->gasColleague)
            ->postJson(route('gas-configuration.reject', $quotation->id), ['approval_note' => 'Data tidak lengkap'])
            ->assertOk();
    ...
    public function test_creator_can_view_show_and_print_pages(): void
    {
        ...
        $this->actingAs($this->creator)
            ->get(route('gas-configuration.show', $quotation->id))
            ->assertOk()
            ->assertSee('Configuration '.$quotation->opportunity->opportunity_name)
            ->assertSee('Quote Gas Treatment');

        $this->actingAs($this->creator)
            ->get(route('gas-configuration.pdf', $quotation->id))
            ->assertOk()
    ...
    public function test_show_honors_back_to_task_parameter(): void
    {
        ...
        $this->actingAs($this->creator)
            ->get(route('gas-configuration.show', ['gas_configuration' => $quotation->id, 'back' => 'task-'.$taskId]))
            ->assertOk()
    ...
    {
        $this->actingAs($this->creator)
            ->get(route('gas-configuration.create'))
            ->assertOk()
            ->assertSee('Buat Gas Configuration')
    ...
    public function test_create_page_only_lists_incomplete_tasks(): void
    {
        ...
        $this->assertStringContainsString('Gas Treatment Tuban', $response->getContent());
    }

    public function test_search_products_returns_only_gas_division_active_products(): void
    {
        ...
        // Produk nama mirip tapi divisi WATER (non-Gas) => tidak boleh tampil
        MasterProduct::create([
            'name' => 'pH::lyser ims',
            'code' => 'E-999-WATER',
            'brand' => 's::can',
        ...
        // Produk nonaktif walau divisi Gas => tidak boleh tampil
        MasterProduct::create([
        ...
        $response = $this->actingAs($this->creator)
            ->getJson(route('gas-configuration.search-products', ['draw' => 1, 'search[value]' => 'pH', 'length' => 100]))
            ->assertOk();
        ...
        $this->actingAs($this->creator)
            ->getJson(route('gas-configuration.search-products', ['draw' => 2, 'search[value]' => 'spectro', 'length' => 100]))
            ->assertOk()
    ...
    public function test_fetch_template_returns_parent_and_children(): void
    {
        ...
        $response = $this->actingAs($this->creator)
            ->getJson(route('gas-configuration.fetch-template', $config->id))
            ->assertOk()
    ...
    public function test_duplicate_config_same_task_same_division_blocked(): void
    {
        ...
        $this->actingAs($this->creator)
            ->postJson(route('gas-configuration.store'), [
                'task_id' => $task->id,
                'parameter_note' => 'pH',
