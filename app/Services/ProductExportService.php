...
{
    public function export($products, array $headers): string
    {
        $filePath = storage_path('app/temp/product-export-'.uniqid().'.xlsx');
        ...
        $zip->addFromString('xl/sharedStrings.xml', $this->sharedStrings($headers, $products));
        $zip->addFromString('xl/worksheets/sheet1.xml', $this->sheetData($headers, $products));
    ...
    private function workbook(): string
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            .'<sheets>'
            .'<sheet name="Product Export" sheetId="1" r:id="rId1"/>'
            .'</sheets>'
            .'</workbook>';
    }
    ...
    private function sharedStrings(array $headers, $products): string
    {
        $strings = [];
        ...
        foreach ($products as $product) {
            foreach ($this->extractRow($product) as $val) {
                $strings[] = (string) $val;
            }
        }
    ...
    private function sheetData(array $headers, $products): string
    {
        $cols = '';
        ...
        foreach ($products as $rowIdx => $product) {
            $r = $rowIdx + 2;
            $xml .= '<row r="'.$r.'">';
            foreach ($this->extractRow($product) as $colIdx => $val) {
    ...
    private function extractRow(object $product): array
    {
        return [
            $product->name ?? '',
            $product->code ?? '',
