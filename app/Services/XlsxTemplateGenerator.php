...
class XlsxTemplateGenerator
{
    private array $headers = [
        'lead_status', 'salutation', 'full_name', 'email', 'job_title',
        'department', 'lead_source', 'lead_title', 'segmentation', 'account_type',
        'follow_up_date', 'mobile', 'phone', 'company', 'field_type',
        'contact_method', 'role_in_project',
        'business_entity', 'business_value', 'interaction_level', 'address_street',
        'city', 'province', 'zip', 'country', 'end_user', 'unqualified_reason',
        'close_date', 'all_field_completed', 'lead_can_be_contacted',
        'lead_appointment', 'need_identification', 'assign_to',
    ];

    private array $exampleRow = [
        'New', 'Bapak', 'John Doe', 'john@example.com', 'Manager',
        'IT', 'Website', 'Software Lead', 'Enterprise', 'Customer',
        '2026-07-01', '08123456789', '021-5551234', 'PT Maju', 'PT',
        'Phone', 'Decision Maker',
        '', '', '', 'Jl. Sudirman 123',
        'Jakarta Selatan', 'DKI Jakarta', '12950', 'Indonesia', '10',
        '', '2026-08-01', '0', '1', '0', '0', '',
    ];
    ...
    public function generate(array $references, string $filePath): void
    {
        ...
        $zip->addFromString('xl/worksheets/sheet1.xml', $this->sheetDataLead());
    ...
    private function workbook(): string
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'.
            ...
            '<sheets>'.
            '<sheet name="Data Lead" sheetId="1" r:id="rId1"/>'.
    ...
    private function sheetDataLead(): string
    {
        $cols = [];
        foreach ($this->headers as $i => $h) {
            $cols[] = '<col min="'.($i + 1).'" max="'.($i + 1).'" width="'.($i < 4 ? 14 : 16).'" customWidth="1"/>';
        }
