...
class IhConfigurationController extends Controller
{
    private const MODULE_CODE = 'MOD_IH_CONFIGURATION';

    public function index()
    {
        return view('ih-configuration.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('ih-configuration.form', [
            'quotation' => null,
            'items' => [],
    ...
    /**
     * Id divisi IH. Kembali null bila divisi belum terdaftar.
     */
    private function ihDivisionId(): ?int
    {
        return Division::where('division_name', 'IH')->value('id');
    }

    /**
     * Daftar configuration divisi IH yang pernah dibuat, dipakai sebagai
     * template isian. Hanya versi terakhir tiap group yang disertakan.
     */
    private function templateList(): array
    {
        $ihId = $this->ihDivisionId();
        if (! $ihId) {
            return [];
        }
    ...
    public function fetchTemplate(Request $request, $id): JsonResponse
    {
        $config = QuoteConfiguration::with(['items'])
            ->where('division_id', $this->ihDivisionId())
            ->findOrFail($id);
    ...
    {
        $divisionId = $this->ihDivisionId();

        // Hanya tampilkan versi terbaru tiap group milik divisi IH.
        $latestIds = QuoteConfiguration::query()
            ->selectRaw('MAX(id) as id')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($quotation->task) {
                $divisionName = $quotation->division?->division_name ?? 'IH';
    ...
    public function edit($id)
    {
        ...
        if ($quotation->status !== QuoteConfiguration::STATUS_DRAFT) {
            return redirect()->route('ih-configuration.index')
    ...
    /**
     * Pencarian produk master_products (hanya aktif milik divisi IH).
    ...
    public function show($id)
    {
        ...
        return view('ih-configuration.show', compact('quotation', 'isSameDivisionApprover', 'back'));
    }
    ...
    public function revise($id): JsonResponse
    {
        ...
        if ($revision->task) {
            $divisionName = $revision->division?->division_name ?? 'IH';
            TaskWorkflowLogger::forTask(
                $revision->task,
                'revise_ih_configuration',
    ...
    {
        $quotation = QuoteConfiguration::where('division_id', $this->ihDivisionId())
            ->findOrFail($id);
        ...
        $versions = QuoteConfiguration::with(['creator', 'finalChecker'])
            ...
            ->map(fn ($v) => [
                ...
                'show_url' => route('ih-configuration.show', $v->id),
            ])
            ->values();
    ...
    public function pdf($id)
    {
        ...
        $pdf = Pdf::loadView('ih-configuration.pdf', compact('quotation'))
    ...
    private function rejectUnknownProducts(array $items): ?JsonResponse
    {
        ...
        return response()->json([
            'success' => false,
            'message' => 'Produk tidak ditemukan di master product divisi IH: #'.$missing->implode(', #').'.',
        ], 422);
    }
