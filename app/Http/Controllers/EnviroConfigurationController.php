...
class EnviroConfigurationController extends Controller
{
    private const MODULE_CODE = 'MOD_ENVIRO_CONFIGURATION';

    public function index()
    {
        return view('enviro-configuration.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('enviro-configuration.form', [
            'quotation' => null,
            'items' => [],
    ...
    /**
     * Id divisi Enviro. Kembali null bila divisi belum terdaftar.
     */
    private function enviroDivisionId(): ?int
    {
        return Division::where('division_name', 'Enviro')->value('id');
    }

    /**
     * Daftar configuration divisi Enviro yang pernah dibuat, dipakai sebagai
     * template isian. Hanya versi terakhir tiap group yang disertakan.
     */
    private function templateList(): array
    {
        $enviroId = $this->enviroDivisionId();
        if (! $enviroId) {
            return [];
        }
    ...
    public function fetchTemplate(Request $request, $id): JsonResponse
    {
        $config = QuoteConfiguration::with(['items'])
            ->where('division_id', $this->enviroDivisionId())
            ->findOrFail($id);
    ...
    {
        $divisionId = $this->enviroDivisionId();

        // Hanya tampilkan versi terbaru tiap group milik divisi Enviro.
        $latestIds = QuoteConfiguration::query()
            ->selectRaw('MAX(id) as id')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($quotation->task) {
                $divisionName = $quotation->division?->division_name ?? 'Enviro';
    ...
    public function edit($id)
    {
        ...
        if ($quotation->status !== QuoteConfiguration::STATUS_DRAFT) {
            return redirect()->route('enviro-configuration.index')
    ...
    /**
     * Pencarian produk master_products (hanya aktif milik divisi Enviro).
    ...
    public function show($id)
    {
        ...
        return view('enviro-configuration.show', compact('quotation', 'isSameDivisionApprover', 'back'));
    }
    ...
    public function revise($id): JsonResponse
    {
        ...
        if ($revision->task) {
            $divisionName = $revision->division?->division_name ?? 'Enviro';
            TaskWorkflowLogger::forTask(
                $revision->task,
                'revise_enviro_configuration',
            ...
            Log::record(
                'revise_enviro_configuration_task',
    ...
    {
        $quotation = QuoteConfiguration::where('division_id', $this->enviroDivisionId())
            ->findOrFail($id);
        ...
        $versions = QuoteConfiguration::with(['creator', 'finalChecker'])
            ...
            ->map(fn ($v) => [
                ...
                'show_url' => route('enviro-configuration.show', $v->id),
            ])
            ->values();
    ...
    public function pdf($id)
    {
        ...
        $pdf = Pdf::loadView('enviro-configuration.pdf', compact('quotation'))
    ...
    private function rejectUnknownProducts(array $items): ?JsonResponse
    {
        ...
        return response()->json([
            'success' => false,
            'message' => 'Produk tidak ditemukan di master product divisi Enviro: #'.$missing->implode(', #').'.',
        ], 422);
    }
